...
const target=process.env.ROYAL_WINE_NJ_KIOSK_URL;
...
try{
  ...
  const response=await context.request.post(origin+'/api/banana',{
    multipart:{image:{name:'approved-reference-test.jpg',mimeType:'image/jpeg',buffer:input},guestCount:'1',style:''},
    timeout:190000
  });
  if(!response.ok()){
