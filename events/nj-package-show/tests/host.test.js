...
test('session endpoint rejects untrusted requests and returns only connection data',async()=>{
  ...
  try {
    ...
    assert.equal((await POST(request({sdp:'v=0\r\nm=audio'}))).status,403);
  }finally{globalThis.fetch=oldFetch;if(previous===undefined)delete process.env.OPENAI_API_KEY;else process.env.OPENAI_API_KEY=previous;if(oldEnv===undefined)delete process.env.VERCEL_ENV;else process.env.VERCEL_ENV=oldEnv;}
});
