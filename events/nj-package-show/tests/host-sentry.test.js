...
test('sentry endpoint validates origin, limits frames, disables storage and returns no image',async()=>{
 ...
 try{
  ...
  let payload;globalThis.fetch=async(_,opts)=>{payload=JSON.parse(opts.body);return Response.json({status:'completed',output:[{type:'message',content:[{type:'output_text',text:JSON.stringify({personPresent:true,greeting:"Hey, you're looking great! Want a Royal Wine photo?"})}]}]});};
