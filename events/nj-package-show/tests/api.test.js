...
import { POST } from '../api/banana.js';
...
async function generateCase({ count = '1', check = valid(Number(count ?? 1)), verifyBody, verifyStatus = 200, verifyThrows = false, duplicate = false, style = '', imageStatus = 200 } = {}) {
  ...
  try {
    ...
    const response = await POST(new Request('https://flow-photo.vercel.app/api/banana', { method: 'POST', body: fd }));
...
for (const count of [1, 2, 3]) {
  test(`exactly ${count} nearest guest(s) approved; one generation and one independent check`, async () => {
    ...
    assert.match(payload.prompt,/actual guest camera photo/);
    assert.match(payload.prompt,/NJ Package Show/);
    assert.match(payload.prompt,/trade-show gallery/i);
    assert.doesNotMatch(payload.prompt,/Damn Good|Old.Fashioned|Sky Terrace|Level 25|South Florida|palm fronds/);
...
test('HTML and text email preserve PowerWyze/client links and retry idempotency', async () => {
  ...
  try {
    ...
    for (const link of ['https://powerwyze.com/', 'https://www.instagram.com/powerwyze/', 'https://royalwine.com/']) {
      assert.ok(calls[0].body.html.includes(link)); assert.ok(calls[0].body.text.includes(link));
    }
    assert.equal(calls[0].body.attachments[0].content_id, 'royal-wine-nj-portrait');
    const key = calls[0].headers.get('idempotency-key');
    assert.ok(key?.startsWith('royal-wine-photo-')); assert.equal(key, calls[1].headers.get('idempotency-key'));
