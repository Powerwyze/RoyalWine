...
const url=process.env.ROYAL_WINE_NJ_KIOSK_URL;
...
try{
  ...
  await ask('There are exactly two people in our photo. Please record that. We are not ready to take it yet.');
  ...
  const mailResponse=page.waitForResponse(r=>new URL(r.url()).pathname==='/api/send-photo');
  await page.locator('#emailConfirm').click();
  const delivered=await mailResponse;
  assert.equal(delivered.status(),200,'Configured email provider must accept the test email');
  ...
  const report=await page.evaluate(()=>({
    ...
    emailConfirmedAndAccepted:!document.getElementById('emailToast').hidden,
