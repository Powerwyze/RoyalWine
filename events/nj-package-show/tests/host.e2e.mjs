...
const allowed=['/host.html','/host.css','/host.js','/host-engine.js','/host-countdown.js','/host-idle.js','/host-captions.js','/host-email.js','/host-avatar.js','/host-sentry.js','/host-sentry-gate.js','/host-sentry-worker.js','/assets/person-detector.tflite','/tests/fixtures/sentry-person.jpg','/email-shortcuts.js','/assets/host-avatar.glb','/assets/trade-background.svg','/assets/royal-wine-logo.png','/assets/bartenura-logo.jpg','/assets/black-irish-logo.png'];
const server=http.createServer(async(req,res)=>{
  ...
  const type={'.html':'text/html','.css':'text/css','.js':'text/javascript','.jpg':'image/jpeg','.svg':'image/svg+xml','.glb':'model/gltf-binary','.png':'image/png','.mjs':'text/javascript','.wasm':'application/wasm','.tflite':'application/octet-stream','.woff2':'font/woff2'}[path.extname(pathname)];
...
await context.route('**/api/health',route=>route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({packetId:'DEMO-009',hostEnabled:true,voiceConfigured:true,imageConfigured:true,emailConfigured:true})}));
...
await page.route('**/api/send-photo',async route=>{emails++;lastEmail=route.request().postDataJSON().email;if(holdEmail)await new Promise(r=>releaseEmail=r);else await new Promise(r=>setTimeout(r,350));await route.fulfill({status:emailFail?502:200,contentType:'application/json',body:JSON.stringify({ok:!emailFail})});});
...
await page.route('**/api/banana',async route=>{
  generations++;
...
try{
  // Missing configuration must never prompt for media or start paid/provider requests.
  const pending=await context.newPage();let providerRequests=0;
  pending.on('request',request=>{if(/\/api\/(host-session|host-greeting|banana|send-photo)/.test(request.url()))providerRequests++;});
  await pending.addInitScript(()=>{window.__mediaRequests=0;navigator.mediaDevices.getUserMedia=async()=>{window.__mediaRequests++;throw new Error('Must not request media before setup');};});
  await pending.route('**/api/health',route=>route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({packetId:'DEMO-009',hostEnabled:true,voiceConfigured:false,imageConfigured:false,emailConfigured:false})}));
  await pending.goto('http://127.0.0.1:4181/host.html');await pending.waitForFunction(()=>document.body.dataset.configuration==='pending');
  assert.equal(await pending.locator('#face').isDisabled(),true);
  ...
  assert.equal(await pending.locator('#face').isDisabled(),true);assert.equal(await pending.evaluate(()=>window.__mediaRequests),0);
  await pending.unroute('**/api/health');await pending.reload();await pending.waitForFunction(()=>document.body.dataset.configuration==='ready');
  ...
  await say(' A relaxed night with your fellow guests.'.repeat(80)+' Enjoy the event!');
  ...
  await fs.writeFile('artifacts/countdown-report.json',JSON.stringify({...timing,duration},null,2));
  await say('While your portrait develops, enjoy the NJ Package Show event with your fellow guests!');
  ...
  assert.ok(await page.evaluate(()=>window.__sent.some(e=>e.type==='session.commentary.append'&&e.content.includes('NJ Package Show'))));
  ...
  await page.route('**/api/host-greeting',async route=>{
    ...
    await route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({personPresent:true,greeting:"Hey, you're looking great in that blue jacket! I'm your AI photo host. Would you like a Royal Wine photo?"})});
  });
  await page.evaluate(()=>{
